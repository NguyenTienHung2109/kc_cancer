...
def find_bbox(image: np.array):
    image = (image.astype(np.uint8)) * 255  # Convert boolean to uint8 (0 and 255)
...
def main(meta_path: str, dataset_dir: str, version: str):
    ...
    for _, row in df.iterrows():
        ...
        if is_clean:
            ...
            study_id = "LIDC-IDRI-" + str(row["patient_id"]).zfill(4)
            mask_name = row["mask_image"]

            study_folder = os.path.join(dataset_dir, "Mask", study_id)
...
if __name__ == "__main__":
    ...
    main(f"data/LIDC/meta_info_one_hot.csv", dataset_dir="data/LIDC", version=2.5)
