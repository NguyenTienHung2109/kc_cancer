...
def negative_sampling(file_path: str):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if not is_clean:
            ...
            if len(pos_bboxes) == 4:
                print("need merge")
                # delete the last pos_bbox 
                pos_bboxes.pop()
                print(len(pos_bboxes))
        else:
            df.at[idx, "left"] = []
            df.at[idx, "top"] = []
            df.at[idx, "width"] = []
...
if __name__ == "__main__":
    ...
    negative_sampling(file_path = f"data/LIDC/train_meta_info_lidc.csv")
    negative_sampling(file_path = f"data/LIDC/val_meta_info_lidc.csv")
    negative_sampling(file_path = f"data/LIDC/test_meta_info_lidc.csv")
