...
from src.models.segmentation.caranet_module import CaraNetModule

rootutils.setup_root(__file__, indicator=".project-root", pythonpath=True)
from src.data.dataset.kc_cancer import KCCancerDataset
...
class KCCancerDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            if self.hparams.train_val_test_meta_file:
                ...
                self.data_train = KCCancerDataset(data_dir=self.hparams.data_dir, 
                                                  meta_file=train_meta_file, 
                                                  images=self.hparams.images,
                                                  cache_data=self.hparams.cache_data)
                self.data_val = KCCancerDataset(data_dir=self.hparams.data_dir, 
                                                meta_file=val_meta_file,
                                                images=self.hparams.images,
                                                cache_data=self.hparams.cache_data)
                self.data_test = KCCancerDataset(data_dir=self.hparams.data_dir, 
                                                 meta_file=test_meta_file,
                                                 images=self.hparams.images,
                                                 cache_data=self.hparams.cache_data)

            else:
                dataset = KCCancerDataset(self.hparams.data_dir, 
...
if __name__ == "__main__":
    ...
    def main(cfg: DictConfig):
        ...
        print(cfg)
        datamodule: KCCancerDataModule = hydra.utils.instantiate(cfg, data_dir=f"{root}/data")
        datamodule.setup()
        ...
        for i in range(n_row):
            ...
            axs[i][2].imshow(images[i, 1], cmap='gray')
            axs[i][2].set_title('Preprocess')
            axs[i][2].axis("off")
