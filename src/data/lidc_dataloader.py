...
from src.data.dataset.lidc_dataset import LIDCDataset
...
class TransformDataset(Dataset):
    ...
    # write a function to check if the bboxes are valid, input is tensor shape (3, 4)
    ...
    def __getitem__(self, idx):
        ...
        if nodules_infos["bbox"].shape[0] == 2:
            # duplicate so that we have shape 3, 4 from 2, 4
            nodules_infos["bbox"] = np.vstack([nodules_infos["bbox"], nodules_infos["bbox"][0]])

        # get label, To-Tensor
        nodules_infos["label"] = {
            "cancer": torch.tensor(nodules_infos["label"][:, :4]),
            "malignancy": torch.tensor(nodules_infos["label"][:, 4:]),
        }
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            if self.hparams.train_val_test_meta_file:
                ...
                self.data_train = LIDCDataset(data_dir=self.hparams.data_dir, 
                                                  meta_file=train_meta_file, 
                                                  cache_data=self.hparams.cache_data)
                self.data_val = LIDCDataset(data_dir=self.hparams.data_dir, 
                                                meta_file=val_meta_file,
                                                cache_data=self.hparams.cache_data)
                self.data_test = LIDCDataset(data_dir=self.hparams.data_dir, 
                                                meta_file=test_meta_file,
                                                cache_data=self.hparams.cache_data)

            else:
                dataset = LIDCDataset(self.hparams.data_dir, 
...
if __name__ == "__main__":
    ...
    def main(cfg: DictConfig):
        ...
        print(cfg)
        datamodule: LIDCDataModule = hydra.utils.instantiate(cfg, data_dir=f"{root}/data")
        datamodule.setup()
        ...
        images, masks, nodules_infos = batch_image

        print("bbox:", nodules_infos["bbox"].shape, nodules_infos["bbox"].dtype)
        print("cancer:", nodules_infos["label"]["cancer"].shape, nodules_infos["label"]["cancer"].dtype)
        print("malignancy:", nodules_infos["label"]["malignancy"].shape, nodules_infos["label"]["malignancy"].dtype)
        ...
        for i in range(n_row):
            ...
            for bbox in nodules_infos["bbox"][i]:
