...
from src.models.components.pretrain.Res2Net_v1b import res2net50_v1b
...
class CaraNet(nn.Module):
    ...
    def forward(self, x):
        ...
        decoder_1 = self.agg1(x4_rfb, x3_rfb, x2_rfb) # bs, 1, 44, 44
        lateral_map_1 = F.interpolate(decoder_1, scale_factor=8, mode='bilinear') # bs, 1, 352, 352
        
        # ------------------- atten-one -----------------------
        decoder_2 = F.interpolate(decoder_1, scale_factor=0.25, mode='bilinear')
        cfp_out_1 = self.CFP_3(x4_rfb) # bs, 32, 11, 11
        ...
        return (lateral_map_5, lateral_map_3, lateral_map_2, lateral_map_1), x2_rfb # x3

if __name__ == '__main__':
    caranet = CaraNet()
    input_tensor = torch.randn(1, 1, 352, 352)

    outputs = caranet(input_tensor)
    
    logits, feature_maps = outputs
    for logit in logits:
        print(logit.shape)
